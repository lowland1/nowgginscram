...
- It runs `./scripts/prepare-scramjet-client.sh` to publish `scramjet.client.js` (auto-clones `vendor/scramjet` when missing and attempts a source build if no bundle is present).
...
- It runs `./scripts/prepare-scramjet-client.sh` so `scramjet.client.js` is published (bootstraps `vendor/scramjet` if needed and attempts a source build when required).
...
2. Ensures Scramjet source exists (submodule update first, clone fallback in prepare script), attempts to build if needed, and copies a client bundle into `/scramjet.client.js` so the browser can load it.
