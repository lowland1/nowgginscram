...
- It runs `./scripts/prepare-scramjet-client.sh` to publish `scramjet.client.js` (auto-clones `vendor/scramjet` when missing and attempts a source build if no bundle is present (pnpm-first, tries recursive/pipeline builds (`-r`, `build:wasm`, `build:rewriter`, `build:client`, `build`), initializes nested submodules, and adds a temporary wasm bridge stub when `rewriter/wasm/out/wasm.js` is missing)).
...
- It runs `./scripts/prepare-scramjet-client.sh` so `scramjet.client.js` is published (bootstraps `vendor/scramjet` if needed and attempts a source build when required (pnpm-first, tries recursive/pipeline builds (`-r`, `build:wasm`, `build:rewriter`, `build:client`, `build`), initializes nested submodules, and adds a temporary wasm bridge stub when `rewriter/wasm/out/wasm.js` is missing)).
...
2. Ensures Scramjet source exists (submodule update first, clone fallback in prepare script), attempts to build if needed (pnpm-first, tries recursive/pipeline builds (`-r`, `build:wasm`, `build:rewriter`, `build:client`, `build`), initializes nested submodules, and adds a temporary wasm bridge stub when `rewriter/wasm/out/wasm.js` is missing), and copies a client bundle into `/scramjet.client.js` so the browser can load it.
