...
- It runs `./scripts/prepare-scramjet-client.sh` to publish `scramjet.client.js` (auto-clones `vendor/scramjet` when missing and attempts a source build if no bundle is present (pnpm-first, tries build:wasm/build:rewriter/build, and initializes nested submodules)).
...
- It runs `./scripts/prepare-scramjet-client.sh` so `scramjet.client.js` is published (bootstraps `vendor/scramjet` if needed and attempts a source build when required (pnpm-first, tries build:wasm/build:rewriter/build, and initializes nested submodules)).
...
2. Ensures Scramjet source exists (submodule update first, clone fallback in prepare script), attempts to build if needed (pnpm-first, tries build:wasm/build:rewriter/build, and initializes nested submodules), and copies a client bundle into `/scramjet.client.js` so the browser can load it.
